...
export default function OrderDetails() {
  ...
  useEffect(() => {
    if (orders) {
      ...
      setOrder(foundOrder);
      if (foundOrder) {
        // Инициализируем editedQuantities с текущими значениями
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      // Проверяем, какие количества изменились
                ...
                <div key={item.productId} className="flex gap-4 pb-6 border-b border-gray-200 last:border-b-0">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      Поставщик: {item.product?.supplierName || "Неизвестно"}
                    </p>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-3 text-sm text-gray-600">
              ...
              <div>
                <p className="text-gray-600">Телефон: {order.shippingAddress.phone}</p>
              </div>
            </div>
          </div>
          ...
          <button
            onClick={() => router.back()}
            className="w-full mt-6 bg-gray-200 text-gray-900 py-3 rounded-lg hover:bg-gray-300 transition font-semibold"
          >
            Вернуться
          </button>
        </div>
